...
- Frontend: `next.config.ts` gains `output: "standalone"` and `outputFileTracingRoot` at the app root (for the sibling `packages/types`); `transpilePackages` unchanged.
...
- Added a repo-root GitHub Actions workflow that builds both images and runs a cross-service smoke test (API healthcheck, liveness, and a CORS + seeded-data check) on PRs touching `apps/agent-health-clinic/**`.
